#!/usr/bin/env python3
"""
Teste final para verificar se os parâmetros da tabela estão sendo aplicados corretamente
"""
...
def verificar_parametros_aplicados(eventos):
    """Verifica se os parâmetros da tabela estão sendo aplicados corretamente"""
    print("\n=== VERIFICAÇÃO DOS PARÂMETROS ===")
    
    # Parâmetros esperados da tabela
    tempo_inicial_esperado = 30  # segundos
    incremento_esperado = 10     # segundos
    duracao_esperada = 10        # segundos
    
    print(f"📋 Parâmetros esperados:")
    print(f"   - Tempo inicial: {tempo_inicial_esperado}s")
    print(f"   - Incremento: {incremento_esperado}s")
    print(f"   - Duração: {duracao_esperada}s")
    ...
    for cpf, eventos_cpf in cpfs.items():
        ...
        # Ordenar por visualização
        eventos_ordenados = sorted(eventos_cpf, key=lambda x: x['visualization'])
        ...
        for i, evento in enumerate(eventos_ordenados):
            ...
            if ':' in delay_str:
                ...
                if len(parts) >= 2:
                    delay_segundos = int(parts[0]) * 60 + float(parts[1])
                else:
                    delay_segundos = float(parts[0])
            else:
                delay_segundos = float(delay_str)
            
            # Calcular delay esperado
            delay_esperado = tempo_inicial_esperado + (i * incremento_esperado)
            
            # Verificar se está correto (com tolerância de 1 segundo)
            if abs(delay_segundos - delay_esperado) <= 1:
                status = "✅"
            else:
                status = "❌"
            
            print(f"   {status} {viz}: {delay_segundos:.1f}s (esperado: {delay_esperado}s)")
    
    # Verificar duração dos eventos
    print(f"\n⏱️  Verificando duração dos eventos:")
    for evento in eventos:
        inicio = evento['hora_exibicao']
        fim = evento['hora_fim']
        
        # Calcular duração
        inicio_dt = datetime.strptime(inicio, '%H:%M:%S.%f')
        fim_dt = datetime.strptime(fim, '%H:%M:%S.%f')
        duracao = (fim_dt - inicio_dt).total_seconds()
        
        if abs(duracao - duracao_esperada) <= 1:
            status = "✅"
        else:
            status = "❌"
        
        print(f"   {status} {evento['visualization']}: {duracao:.1f}s (esperado: {duracao_esperada}s)")
...
def main():
    print("🧪 TESTE FINAL - VERIFICAÇÃO DOS PARÂMETROS")
    print("=" * 60)
    ...
    if eventos:
        ...
        verificar_parametros_aplicados(eventos)
